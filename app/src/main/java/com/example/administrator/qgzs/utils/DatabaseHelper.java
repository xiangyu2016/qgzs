...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        //升级GoodsBean表，添加性别
        //db.execSQL("ALTER TABLE GoodsBean ADD COLUMN gender VARCHAR(2)");
    }
    ...
    public void updateAGoods(Goods bean) {
        ...
        //根据id更新一条数据
        database.execSQL(
                "UPDATE GoodsBean SET goodsID=?, price=?, WHERE id=?",
                new Object[]{bean.getGoodsID(), bean.getPrice(), bean.getId()});
    }
     ...
     * @param id    用户id
     */
    public void deleteAGoodsBean(Integer id) {
        SQLiteDatabase database = getWritableDatabase();
        //根据id删除一条数据
        database.execSQL("DELETE FROM GoodsBean WHERE id=?",
                new Object[]{id});
    }
     ...
     * @return
     */
    public List<Goods> readAllGoodsBean() {
        SQLiteDatabase database = getReadableDatabase();
        Cursor cursor = database.rawQuery("SELECT * FROM GoodsBean", new String[]{});
     ...
     * @return      用户对象
     */
    public Goods readAGoodsBean(Integer id) {

        //如果只对数据进行读取，建议使用此方法
        SQLiteDatabase database = getReadableDatabase();
        Cursor cursor  = database.rawQuery(
                "SELECT * FROM GoodsBean WHERE id=?",
